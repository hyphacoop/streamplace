...
func TestIngredientConcat(t *testing.T) {
	withNoGSTLeaks(t, func() {
		...
		require.NoError(t, err)
		unsignedBs, err := rws.Bytes()
		require.NoError(t, err)
		signedRWS := aqio.NewReadWriteSeeker([]byte{})
		err = ms.SignConcatMP4(context.Background(), bytes.NewReader(unsignedBs), ingredients, signedRWS)
		require.NoError(t, err)
		signedConcatBS, err := signedRWS.Bytes()
		require.NoError(t, err)
